...
- **스트림(stream)**: 데이터를 순차적으로 읽고 쓸 수 있게 추상화된 객체. 
	매 호출마다 "어디서부터 읽을지"를 호출자가 명시하지 않아도, 이전 호출이 끝난 지점에서 자동으로 이어서 처리됨.
	

```C
	// 랜덤 액세스 (배열, 메모리)
	char buf[1000];
	char a = buf[0];     // 위치를 매번 명시
	char b = buf[500];   // 순서 상관없이 아무 데나 점프 가능, "이전 호출"과 무관

	// 순차 접근 (스트림)
	FILE *fp = fopen("data.txt", "r");
	char c1 = fgetc(fp);  // 위치 지정 안 함. 그냥 "다음 거 줘"
	char c2 = fgetc(fp);  // c1을 읽은 지점 바로 다음부터 자동으로 이어짐
```
